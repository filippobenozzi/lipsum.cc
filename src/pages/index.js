...
const IndexPage = ({ data }) => {
  ...
  const [morgan, setMorgan] = useState(false)
  ...
  let sentenceMorgan = function() {
    return _.sample(dict.morgan).replace(/\{\{([a-z]+)\}\}/gi, (match, category) => _.sample(dict[category]))
  };
  ...
  let paragraph = function() {
    if (phrases) {
      ...
      return _.join(_.times(sentence_count, sentenceMorgan), " ")
    } else {
      return loremIpsum({
        units: "sentences",
      ...
      <form>
        ...
        <div>
          <input
              ...
              type="checkbox"
              checked={morgan}
              onChange={e => setMorgan(!morgan)}
          /> <label htmlFor="morgan">Morgan</label>
        </div>
      </form>
      <div>
      {_.times(count,() =>
...
export const pageQuery = graphql`
  query ProjectsQuery {
    site {
      siteMetadata {
        dict {phrase, morgan, role, noun, adjective, verb}
      }
    }
  }
`
